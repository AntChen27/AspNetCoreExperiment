...
namespace Antc.ApiConsul.Extensions
{
    public static class ServiceExtensions
    {
        ...
        /// <summary>
        /// 使用consul
        /// 默认的健康检查接口格式是 http://host:port/api/healthCheck
        /// </summary>
        ...
        public static IApplicationBuilder UseConsul(this IApplicationBuilder app)
        {
            ...
            foreach (var item in addressIpv4Hosts)
            {
                ...
                ConsulServiceRegister(consul, registration, appLife);
            ...
            ConsulServiceRegister(consul, localhostregistration, appLife);
        ...
        private static void ConsulServiceRegister(IConsulClient consul, AgentServiceRegistration serviceRegistration, IApplicationLifetime appLife)
        {
            try
            {
                consul.Agent.ServiceRegister(serviceRegistration).GetAwaiter().GetResult();
            }
            catch (Exception e)
            {
                Console.WriteLine($"向Consul注册服务失败{e}");
                throw new Exception("向Consul注册服务失败", e);
            }
            ...
            {
                consul.Agent.ServiceDeregister(serviceRegistration.ID).GetAwaiter().GetResult();
            });
        }
    }
}